...
const ServicePopup = ({  passenger, isOpen, onClose, onAddServices }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (storedWagon) {
        ...
        axios
          .get(`http://localhost:9001/additional-services/wagon/${wagonId}`)
          .then((response) => {
  ...
  const handleAddServices = () => {
    ...
    localStorage.setItem(`selectedServices_${passenger.id}`, JSON.stringify(selectedServices));
  };
          ...
          {services.map((service) => (
            <li key={service.id}>
              ...
              <input
                type="checkbox"
                onChange={(e) =>
                  handleQuantityChange(service.id, e.target.checked)
                }
